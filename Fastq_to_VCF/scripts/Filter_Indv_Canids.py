...
### Delete weird fields
	#if re.match(r'GT', line[8]): print(line)
	#if line[8] == '\tGT\t' or 'GT:AD:DP': print(line) #outVCF.write('%s\t%s\t%s\n' % ('\t'.join(line[0:6]), 'FAIL_FewFields', '\t'.join(line[7:])) ); c$
	A = line[8].split(':')
...
### Filter out sites with missing/failing genotype
	...
	#if missing2>0: outVCF.write('%s\t%s\t%s\n' % ('\t'.join(line[0:6]), 'FAIL_missing', '\t'.join(line[7:])) ); continue
	#print(line[8])
	#if line[8] == 'GT': print(line) #outVCF.write('%s\t%s\t%s\n' % ('\t'.join(line[0:6]), 'FAIL_FewFields', '\t'.join(line[7:])) ); continue
	#continue #print(line) 
